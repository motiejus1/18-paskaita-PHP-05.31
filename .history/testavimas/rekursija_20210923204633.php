...
function kategorijuMedis($tevinis_id = 0, $kategorijos_medis_masyvas = '') {
    ...
    $sql = "SELECT * FROM kategorijos WHERE tevinis_id = $tevinis_id"; // grazins paciu rekursijos veikmo momentu
    // visas tevines kategorijas

    //masyva - atvaizdavimui. html tagus , kurie mums masyva atvaizduos kaip kategoriju medi
    $kategorijos_medis_masyvas[] = "<ul>";

    // foreach($kategorijuMasyvas as $kategorija) {
    //     $kategorijos_medis_masyvas[] = "<li>". $kategorija["pavadinimas"]."</li>";
    //     $kategorijos_medis_masyvas[] = kategorijuMedis($kategorija["ID"], $kategorijos_medis_masyvas );
    // }
